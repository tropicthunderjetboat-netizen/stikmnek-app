-- Fix admin approval:
-- 1) Copy listing email + set active = true on approve.
-- 2) Merge into existing onboarding stub (active = false) for same owner instead of inserting a second row.
-- 3) Preserve nullable pricing_tiers (non-tiered listings).
...
BEGIN
  ...
  IF p_decision = 'approved' THEN
    ...
    -- When business_offerings exists (Phase 2 migration applied), keep the primary row in sync.
