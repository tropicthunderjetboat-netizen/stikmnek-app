-- Ensure approved pending submission photos are linked to the approved offering.
-- Without this, `business_photos.offering_id` stays NULL and legacy fallbacks can pool photos
-- under the first/primary offering (exact symptom: first listing accumulates all galleries).
...
BEGIN
  ...
  IF p_decision = 'approved' THEN
    -- Existing profile: update primary offering or create missing one.
    IF v_pending.business_id IS NOT NULL THEN
      ...
      -- ✅ Critical fix: stamp approved photos onto the offering that was just approved/updated.
    ...
    -- New business approval (create business + ensure at least one offering).
    v_desc := COALESCE(v_pending.description, '');
    v_email := NULLIF(TRIM(COALESCE(v_pending.email, '')), '');
    ...
    -- Resolve primary offering id for this new business and stamp photos onto it.
    SELECT o.id INTO v_primary_offering_id
    FROM public.business_offerings o
