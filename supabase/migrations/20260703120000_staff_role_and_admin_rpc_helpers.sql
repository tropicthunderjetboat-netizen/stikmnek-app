-- Staff onboarding role: limited admin panel (Onboard + Businesses + Approvals).
-- Staff email allowlist must stay in sync with src/lib/adminRoles.ts and _shared/adminRoles.ts.

CREATE OR REPLACE FUNCTION public.is_full_app_admin(p_uid uuid DEFAULT auth.uid())
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE up.user_id = p_uid
          AND (up.role = 'admin' OR up.user_type = 'admin')
      )
    );
$$;

CREATE OR REPLACE FUNCTION public.is_staff_or_app_admin(p_uid uuid DEFAULT auth.uid())
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT
    public.is_full_app_admin(p_uid)
    OR (
      p_uid IS NOT NULL
      AND (
        EXISTS (
          SELECT 1 FROM auth.users u
          WHERE u.id = p_uid
            AND lower(u.email) IN ('stikmnekstaff@gmail.com')
        )
        OR EXISTS (
          SELECT 1 FROM public.user_profiles up
          WHERE up.user_id = p_uid
            AND (up.role = 'staff' OR up.user_type = 'staff')
        )
      )
    );
$$;

GRANT EXECUTE ON FUNCTION public.is_full_app_admin(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_staff_or_app_admin(uuid) TO authenticated;

-- ─── RPCs used by staff (onboard / businesses / approvals) ───
...
-- Patch review_pending_business admin gate only (body unchanged from 20260519120100).
    ...
    IF to_regclass('public.business_offerings') IS NOT NULL THEN
      IF EXISTS (SELECT 1 FROM public.business_offerings o WHERE o.business_id = v_new_biz_id) THEN
        UPDATE public.business_offerings o SET
          ...
          whatsapp_number = v_pending.whatsapp_number,
          pricing_tiers = v_pending.pricing_tiers,
          hours = COALESCE(NULLIF(TRIM(v_pending.hours), ''), o.hours, ''),
          tags = ARRAY[COALESCE(NULLIF(TRIM(v_pending.category), ''), 'dining')],
          active = true,
          updated_at = now()
        WHERE o.id = (
...
COMMENT ON FUNCTION public.is_staff_or_app_admin(uuid) IS
  'True for full admins or onboarding staff (role/email allowlist).';

-- After creating auth user stikmnekstaff@gmail.com in Supabase Dashboard:
-- UPDATE public.user_profiles SET role = 'staff', user_type = 'staff' WHERE email = 'stikmnekstaff@gmail.com';
