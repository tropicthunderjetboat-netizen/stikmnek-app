-- When pending_businesses.business_id is set (edit / resubmit on existing profile),
-- UPDATE the primary business_offerings row instead of always INSERTing a duplicate.
-- Also sync public.businesses stub fields from pending so legacy/admin UIs stay aligned.
-- Narrow "stub approval" offering UPDATE to the oldest offering (created_at) per profile.
