-- ═══════════════════════════════════════════════════════════════
-- Superstar Review Support
-- Run this in Supabase SQL Editor or via supabase db push
-- ═══════════════════════════════════════════════════════════════

-- 1. Allow rating 6 in public.reviews (Superstar reviews)
ALTER TABLE public.reviews DROP CONSTRAINT IF EXISTS reviews_rating_check;
ALTER TABLE public.reviews ADD CONSTRAINT reviews_rating_check CHECK (rating >= 1 AND rating <= 6);

-- 2. Add superstar_credits to user_profiles (tracks purchased Superstar reviews)
ALTER TABLE public.user_profiles
ADD COLUMN IF NOT EXISTS superstar_credits integer NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.user_profiles.superstar_credits IS 'Number of Super Star (6-star) review credits purchased. Decremented when user submits a Super Star review.';

-- 3. RPC: Submit Superstar review (atomic: decrement credit + insert review)
-- Call via: supabase.rpc('submit_superstar_review', { p_business_id, p_user_name, p_comment })
...
-- Call via: supabase.rpc('increment_superstar_credits', { p_user_id: '...' })
-- When called by authenticated user without p_user_id, uses auth.uid()
