...
const AuthModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setShowAuth(false)} />
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden max-h-[95vh] overflow-y-auto">
        {/* Loading overlay when auth is processing (e.g. waiting for onAuthStateChange after sign-in) */}
        {authLoading && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/90 rounded-2xl">
            <div className="flex flex-col items-center gap-3">
              <Loader2 className="w-10 h-10 text-teal-600 animate-spin" />
              <p className="text-sm font-medium text-gray-600">
                {language === 'en' ? 'Signing you in...' : language === 'fr' ? 'Connexion en cours...' : 'Yu saen in...'}
              </p>
            </div>
          </div>
        )}
