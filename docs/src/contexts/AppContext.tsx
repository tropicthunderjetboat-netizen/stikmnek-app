...
// Add your admin email(s) here if different from admin@stikmnek.com
// ═══════════════════════════════════════════════════════════════
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const signInSafetyTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const handleAuthenticatedUser = useCallback(async (
    ...
    try {
      ...
      if (signInSafetyTimerRef.current) {
        clearTimeout(signInSafetyTimerRef.current);
        signInSafetyTimerRef.current = null;
      }
      setAuthLoading(false);
    }
  ...
  // Per-sign-in safety: if SIGNED_IN never fires within 8s, clear authLoading.
  // ═══════════════════════════════════════════════════════════
  const SIGN_IN_SAFETY_MS = 8000;
  ...
  const signIn = useCallback(async (email: string, password: string) => {
    ...
    // Per-sign-in safety: if handleAuthenticatedUser never runs (SIGNED_IN doesn't fire),
    // force authLoading false after 8s so user isn't stuck indefinitely
    if (signInSafetyTimerRef.current) clearTimeout(signInSafetyTimerRef.current);
    signInSafetyTimerRef.current = setTimeout(() => {
      signInSafetyTimerRef.current = null;
      setAuthLoading(prev => {
        if (prev) {
          console.warn('[signIn] Safety timer: authLoading still true after 8s — forcing false');
          toast.info('If you signed in successfully, please refresh the page.');
          return false;
        }
        return prev;
      });
