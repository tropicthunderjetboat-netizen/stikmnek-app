...
const BusinessGrid: React.FC<BusinessGridProps> = ({ showFeaturedOnly = false, title }) => {
  ...
  const filtered = useMemo(() => {
    ...
    switch (sortBy) {
      case 'leaderboard':
        result.sort((a, b) => {
          const scoreA = computeLeaderboardScore(a, allBusinesses, dbReviews, redemptions).score;
          const scoreB = computeLeaderboardScore(b, allBusinesses, dbReviews, redemptions).score;
  ...
  const categoryIcons: Record<string, React.ReactNode> = {
    ...
    transportation: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.4 2.9A3.5 3.5 0 002 12.2V15c0 .6.4 1 1 1h2" />
